...
- (void)viewDidLoad {
    ...
    //设置默认起始时间点
    _startTime = 0;
//    AVURLAsset *urlSet = [AVURLAsset assetWithURL:_videoUrl];
//    CMTime time = [urlSet duration];
//    seconds = ceil(time.value/time.timescale);
    seconds = [TJMediaManager getVideoTimeWithURL:_videoUrl];
    _endTime = seconds;
    //    [self preViewAction];
    _playTime = 0;
...
-(void)setupUI{
    ...
    //demo中的按钮
    UIButton *cutBtn = [[UIButton alloc]initWithFrame:CGRectMake(self.view.bounds.size.width-80, self.view.bounds.size.height - 50-kSafeAreaBottomHeight,80, 40)];
//    [cutBtn setImage:[UIImage imageNamed:@"channleGou"] forState:UIControlStateNormal];
    ...
    [cutBtn setTitleColor:RBColor(48, 143, 195) forState:UIControlStateNormal];
    _cutBtn = cutBtn;
    [self.view addSubview:cutBtn];
    
    UIButton *backBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, self.view.bounds.size.height - 50-kSafeAreaBottomHeight, 80, 40)];
//    [backBtn setImage:[UIImage imageNamed:@"videoClose"] forState:UIControlStateNormal];
  ...
  [backBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    ...
    //裁剪操作控制区
    TimeChooseView *chooseView = [[TimeChooseView alloc]initWithFrame:CGRectMake(0, self.view.bounds.size.height - 120-kSafeAreaBottomHeight,self.view.bounds.size.width,50)];
...
-(void)preViewAction{
    ...
    [_player seekToTime:CMTimeMake(_startTime*seconds, seconds) toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero completionHandler:^(BOOL finished) {
        [_player play];
    }];
...
//            }];
          ...
          __strong typeof(self) strongself = weakself;
          if (strongself.cutDoneBlock) {
            strongself.cutDoneBlock(exportPath);
          }
...
-(void)jumpToTime:(CGFloat )time{
    ...
    [_player pause];
    [_player seekToTime:CMTimeMake(time*seconds, seconds) toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero completionHandler:^(BOOL finished) {
...
-(void)playbackFinished{
    ...
    // 播放完成后重复播放
    // 跳到剪切开始处
    [_player seekToTime:CMTimeMake(_startTime*seconds, seconds)];
    [_player play];
}
