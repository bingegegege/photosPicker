...
- (void)viewDidLoad
{
    ...
    if (_photographFlag) {
        ...
        _yjVideoView.yjSeconds = 60;
    }
  if(_locationStr.length > 0)
  {
    NSString *locationStr = _locationStr;
    ...
    NSAttributedString *attributedText = [[NSAttributedString alloc] initWithString:locationStr attributes:@{NSShadowAttributeName:shadow}];
    
    _yjVideoView.labs.attributedText = attributedText;
  }
...
//        NSData *data = [NSData dataWithContentsOfURL:imageData];
//        NSInteger videoSize = data.length/1024;
//        float sizes = (float)videoSize/1024;
