...
@property (nonatomic,strong) UIView *waterView; // 水印视图
...
- (void)setupSubviews {
    ...
    [self.waterView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_offset(CGSizeMake(207, 35));
    }];
  
    self.labs = [[UILabel alloc] initWithFrame:CGRectMake(53, 0, 142, 35)];
    self.labs.font = [UIFont fontWithName:@"Helvetica-Bold" size:12];
    self.labs.textColor = [UIColor whiteColor];
    self.labs.numberOfLines = 2;
    self.labs.textAlignment = NSTextAlignmentLeft;
    [self.waterView addSubview:self.labs];
  
  
  
  UIImageView *imageV = [[UIImageView alloc] init];
  imageV.image = [UIImage imageNamed:@"video_logo.png"];
  imageV.frame = CGRectMake(0, 0, 36, 36);
  // 将水印放在一个跟视频大小相等的View上
  
  UIView *lineV = [[UIView alloc] init];
  lineV.backgroundColor = [UIColor whiteColor];
  lineV.frame = CGRectMake(42, 2, 2, 30);
  UIBezierPath *shadowPath = [UIBezierPath
                              bezierPathWithRect:lineV.bounds];
  lineV.layer.masksToBounds = NO;
  lineV.layer.shadowColor = [UIColor blackColor].CGColor;
  lineV.layer.shadowOffset = CGSizeMake(0.0f, 5.0f);
  lineV.layer.shadowOpacity = 0.5f;
  lineV.layer.shadowPath = shadowPath.CGPath;
  
  [self.waterView addSubview:imageV];
  
    [self.waterView addSubview:lineV];
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    NSLog(@"视频录制时间完成=%ld,%ld",60-self.seconds,self.yjSeconds);
...
- (void)changeViewLayout {
    ...
    NSLog(@"录制时间=%ld！%ld",self.seconds,self.yjSeconds);
  if(isOver)
  {
  ...
  {
    if(60-self.seconds<3 || self.seconds == 0)
    {
      NSLog(@"录制时间不能少于3s，请重新录制！");
      self.photographImageView.userInteractionEnabled = NO;
...
- (UIView *)waterView {
    if (!_waterView) {
        _waterView = [UIUtility initImageViewWithImageName:@""];
...
- (UILabel *)hintTitleLB {
    if (!_hintTitleLB) {
        _hintTitleLB = [UIUtility initLabelWithText:@"按住录像，松开结束" fontSize:FontSize15 color:APP_WHITE textAlignment:NSTextAlignmentCenter];
    }
    return _hintTitleLB;
}
