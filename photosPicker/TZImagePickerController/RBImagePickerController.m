...
//  更多信息，请前往项目的github地址：https://github.com/banchichen/RBImagePickerController
...
- (void)configBarButtonItemAppearance {
    ...
    if (@available(iOS 9, *)) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[RBImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[RBImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    RBAlbumPickerController *albumPickerVc = [[RBAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            NSDictionary *infoDict = [RBCommonTools tz_getInfoDictionary];
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    [RBImagePickerConfig sharedInstance].showSelectedIndex = showSelectedIndex;
}

- (void)setShowPhotoCannotSelectLayer:(BOOL)showPhotoCannotSelectLayer {
    _showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
    [RBImagePickerConfig sharedInstance].showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
}

- (void)setNotScaleImage:(BOOL)notScaleImage {
    _notScaleImage = notScaleImage;
    [RBImagePickerConfig sharedInstance].notScaleImage = notScaleImage;
}

- (void)setNeedFixComposition:(BOOL)needFixComposition {
    _needFixComposition = needFixComposition;
    [RBImagePickerConfig sharedInstance].needFixComposition = needFixComposition;
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[TZImageManager manager] authorizationStatusAuthorized]) {
        ...
        RBAlbumPickerController *albumPickerVc = (RBAlbumPickerController *)self.visibleViewController;
        if ([albumPickerVc isKindOfClass:[RBAlbumPickerController class]]) {
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    RBAlbumPickerController *albumPickerVc = [self.childViewControllers firstObject];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [RBImagePickerConfig sharedInstance].allowPickingImage = allowPickingImage;
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [RBImagePickerConfig sharedInstance].allowPickingVideo = allowPickingVideo;
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    [RBImagePickerConfig sharedInstance].preferredLanguage = preferredLanguage;
    [self configDefaultBtnTitle];
}

- (void)setLanguageBundle:(NSBundle *)languageBundle {
    _languageBundle = languageBundle;
    [RBImagePickerConfig sharedInstance].languageBundle = languageBundle;
    [self configDefaultBtnTitle];
}
...
@interface RBAlbumPickerController ()<UITableViewDataSource,UITableViewDelegate> {
    UITableView *_tableView;
}
@property (nonatomic, strong) NSMutableArray *albumArr;
@end

@implementation RBAlbumPickerController
...
- (void)viewDidLoad {
    ...
    RBImagePickerController *imagePickerVc = (RBImagePickerController *)self.navigationController;
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    RBImagePickerController *tzImagePicker = (RBImagePickerController *)self.navigationController;
    if (tzImagePicker && [tzImagePicker isKindOfClass:[RBImagePickerController class]]) {
...
- (void)viewDidLayoutSubviews {
    ...
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden) top += [RBCommonTools tz_statusBarHeight];
...
@implementation RBImagePickerConfig

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static RBImagePickerConfig *config = nil;
    dispatch_once(&onceToken, ^{
        if (config == nil) {
            config = [[RBImagePickerConfig alloc] init];
